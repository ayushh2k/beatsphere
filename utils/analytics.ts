...
class AnalyticsService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.username = await SecureStore.getItemAsync('lastfm_username');
      ...
      if (this.sessionId) {
        await SecureStore.setItemAsync('analytics_session_id', this.sessionId);
      }
   ...
   * Track chat message
   */
  async trackChatMessage(messageType: 'text' | 'gif', chatType: 'global' | 'direct'): Promise<void> {
  ...
  private async getOrCreateDeviceId(): Promise<string> {
    let deviceId = await SecureStore.getItemAsync('analytics_device_id');

    if (!deviceId) {
      // Generate UUID v4
      deviceId = this.generateUUID();
      await SecureStore.setItemAsync('analytics_device_id', deviceId);
    }
